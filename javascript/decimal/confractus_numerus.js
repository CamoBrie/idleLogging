...
! function(t, n) {
	...
	var t = function(t, n, e) {
			...
			var i = String(t),
				r = "number" == typeof n ? n : parseInt(n, 10);
		...
		o = function() {
			function o(t) {
				this.mantissa = NaN, this.exponent = NaN, t instanceof o ? this.fromDecimal(t) : "number" == typeof t ? this.fromNumber(t) : "string" == typeof t ? this.fromString(t) : (this.mantissa = 0, this.exponent = 0)
			}
			return Object.defineProperty(o.prototype, "m", {
			...
			}, o.fromDecimal = function(t) {
				return (new o).fromDecimal(t)
				...
				var r = n.add(i.mul(e)).sub(e.div(2)),
					o = r.pow(2);
			...
			}, o.prototype.fromDecimal = function(t) {
				...
				} else if ("NaN" === t) this.mantissa = Number.NaN, this.exponent = Number.NaN;
				else if (this.fromNumber(parseFloat(t)), isNaN(this.mantissa)) throw Error("[DecimalError] Invalid argument: " + t);
				return this
			}, o.prototype.fromValue = function(t) {
				return t instanceof o ? this.fromDecimal(t) : "number" == typeof t ? this.fromNumber(t) : "string" == typeof t ? this.fromString(t) : (this.mantissa = 0, this.exponent = 0, this)
				...
				var n = t + 1,
					e = Math.ceil(Math.log10(Math.abs(this.mantissa))),
				...
				isFinite(n) || (n = 17);
				var e = n + 1,
					i = Math.max(1, Math.ceil(Math.log10(Math.abs(this.mantissa))));
				...
				var n, e = t instanceof o ? t.toNumber() : t,
					r = this.exponent * e;
	...
	*/
	class Decimal {
		static fromValue(value) {
			if (value == null) {
				return {
					logarithm: Number.NEGATIVE_INFINITY
				}
				...
				if (split1Safe == Number.POSITIVE_INFINITY || split1Safe == Number.NEGATIVE_INFINITY) {
					if (split[0] < 0) return {
						logarithm: Number.NaN
					}
					return {
						logarithm: Decimal_BI.fromString(split[1])
					}
				}
				if (split[0] == '') return {
					logarithm: split1Safe
				}
				else {
					split[0] = parseFloat(split[0])
					return {
						logarithm: Math.log10(split[0]) + split1Safe
					}
				}
		...
		static expHelperAdd(e1, e2) {
			if (typeof(e1) == "number" && typeof(e2) == "number") {
				...
				if (res == Number.POSITIVE_INFINITY || res == Number.NEGATIVE_INFINITY) {
					if (e1 == Number.POSITIVE_INFINITY) {} else if (e1 == Number.NEGATIVE_INFINITY) {} else if (e2 == Number.POSITIVE_INFINITY) {} else if (e2 == Number.NEGATIVE_INFINITY) {} else hardCalc = true
				}
			...
			if (exponent == 308)
				if (mantissa > -1.7976931348623157 && mantissa < 1.7976931348623157) return res.toNumber()
			if (exponent == 9e15) {
		...
		static expHelperPow(e, ep) {
			if (typeof(e) == "number") {
				...
				var hardCalc = false
				if (res == Number.POSITIVE_INFINITY || res == Number.NEGATIVE_INFINITY) {
					if (e == Number.POSITIVE_INFINITY) {} else if (e == Number.NEGATIVE_INFINITY) {} else if (ep == Number.POSITIVE_INFINITY) {} else if (ep == Number.NEGATIVE_INFINITY) {} else hardCalc = true
				}
		...
		get mantissaAndExponent() {
			if (this.logarithm instanceof Decimal_BI) return {
				m: 1,
				e: this.logarithm
			}
			if (this.logarithm == Number.NEGATIVE_INFINITY) return {
				m: 0,
				e: 0
			}
			var logInt = Math.floor(this.logarithm)
			return {
				m: Math.pow(10, this.logarithm - logInt),
				e: logInt
			}
		}
		get e() {
